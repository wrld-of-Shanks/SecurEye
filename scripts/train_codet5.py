...
class FixDataset(Dataset):
    def __init__(self, vulnerable_codes, fixed_codes, tokenizer, max_length=128):
...
def train_codet5():
    print("=" * 60)
    print("TRAINING CODET5 FIX GENERATOR")
    print("=" * 60)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    csv_path = '/Users/shanks/Desktop/SentinelAI/data/code/fixes_dataset.csv'
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                for i, output in enumerate(outputs):
                    ...
                    reference = val_fix[batch_idx * 8 + i] if batch_idx * 8 + i < len(val_fix) else ""
    ...
    save_path = '/Users/shanks/Desktop/SentinelAI/services/code/models/weights/codet5_fixer'
    os.makedirs(save_path, exist_ok=True)
    model.save_pretrained(save_path)
    tokenizer.save_pretrained(save_path)
    print(f"\nModel saved to: {save_path}")
    print("Note: Using RobertaTokenizer for CodeT5 (compatible architecture)")
